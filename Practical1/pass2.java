...
public class pass2 {
	public static void main(String[] Args) throws IOException{
	     ...
	     FileWriter f1 = new FileWriter("PassOp.txt");
		 ...
		 while ((s = b2.readLine()) != null) {
			String[] word = s.split("\\s+"); // Split by any whitespace
			symSymbol.put(symtabPointer++, word[1].trim()); // Trim to remove extra spaces
		}
		
	     
		 while ((s = b3.readLine()) != null) {
			String[] word = s.split("\\s+"); // Split by any whitespace
			litSymbol.put(littabPointer, word[0].trim());
			litAddr.put(littabPointer++, word[1].trim());
		}
